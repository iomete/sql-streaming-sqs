...
class SqsClient(sourceOptions: SqsSourceOptions,
  ...
  private def createSqsClient(): AmazonSQS = {
    try {
      val roleArn = Option(hadoopConf.getTrimmed("fs.s3a.assumed.role.arn"))

      val awsCredentialsProvider = roleArn match {
        case Some(_) => new AssumedRoleCredentialProvider(null, hadoopConf)
      ...
      case e: Exception =>
        throw new SparkException(s"Error occurred while creating Amazon SQS Client", e)
    }
  }
